...
def create_cudart_func_map(func_keys, is_musa):
    mapping = {}
    for key in func_keys:
        if is_musa and key.startswith("cuda"):
            mapped_name = "musa" + key[4:]
            mapping[key] = mapped_name
        else:
            mapping[key] = key
    return bidict(mapping)

cudart_func_map = create_cudart_func_map(_cudart_func_keys, _is_musa)
